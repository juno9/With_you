...
public class Register extends AppCompatActivity {
    ...
    NidOAuthLogin 네이버로그인 = new NidOAuthLogin();
    EditText PW입력;
    EditText PW확인입력;
    EditText 이름입력;
    EditText 이메일입력;
    ...
    protected void onCreate(Bundle savedInstanceState) {//최초 빌드때 실행
        ...
        SharedPreferences 이벤트쉐어드 = getSharedPreferences("이벤트쉐어드프리퍼런스", MODE_PRIVATE);//데이터 가져올 쉐어드 선언
        SharedPreferences.Editor 이벤트쉐어드에디터 = 이벤트쉐어드.edit();
...
//                if (pattern.matcher(email).matches()) {//입력한 이메일이 형식에 맞는지 확인
                    if (PW.equals(PW확인)) {
                        ...
                        JSONObject 나의이벤트제이슨 = new JSONObject();
                        JSONObject 상대이벤트제이슨 = new JSONObject();
                        JSONObject 나의앨범제이슨 = new JSONObject();
                        try {
                        ...
                        String 나의이벤트제이슨스트링 = 나의이벤트제이슨.toString();//위의 값들을 가진 제이슨 데이터를 스트링으로 변환
                        String 나의앨범제이슨스트링 = 나의앨범제이슨.toString();
                        ...
                        이벤트쉐어드에디터.putString(이메일, 나의이벤트제이슨스트링);
                        이벤트쉐어드에디터.apply();
                        앨범쉐어드에디터.putString(이메일, 나의앨범제이슨스트링);
                        앨범쉐어드에디터.apply();
                        finish();
